...
export const ACTION_TYPES = {
  ...
  PARTIAL_UPDATE_PACIENTE: 'paciente/PARTIAL_UPDATE_PACIENTE',
...
export default (state: PacienteState = initialState, action): PacienteState => {
  switch (action.type) {
    ...
    case REQUEST(ACTION_TYPES.PARTIAL_UPDATE_PACIENTE):
      return {
        ...state,
        errorMessage: null,
        updateSuccess: false,
    ...
    case FAILURE(ACTION_TYPES.PARTIAL_UPDATE_PACIENTE):
    ...
    case SUCCESS(ACTION_TYPES.PARTIAL_UPDATE_PACIENTE):
      return {
        ...state,
        updating: false,
        updateSuccess: true,
...
export const getEntities: ICrudGetAllAction<IPaciente> = (page, size, sort) => {
  ...
  return {
    ...
    payload: axios.get<IPaciente>(`${requestUrl}${sort ? '&' : '?'}cacheBuster=${new Date().getTime()}`),
  };
};
...
export const partialUpdate: ICrudPutAction<IPaciente> = entity => async dispatch => {
  const result = await dispatch({
    type: ACTION_TYPES.PARTIAL_UPDATE_PACIENTE,
